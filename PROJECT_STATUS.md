...
**Last updated:** 2026-07-24 (AI feature track — worker now runs persistently, existing documents backfilled, polling has a timeout; found a real pre-existing gap: most seeded demo documents have no actual file on disk)
...
- 🟨 **AI feature track (new, separate from the §19 hackathon roadmap)** — planned end-to-end (see the published "Smart Rename & Auto-Tag — Gemini Rollout Plan": phased rollout starting in shadow mode, `app/ai/` provider-abstraction architecture, Postgres-backed job queue, prompt/token strategy, UX). Text extraction (classic parsing, not an AI call) is fully built, wired, and has its **first user-facing surface**: a quiet "Analyzing document…" badge on Document Details, visible only while extraction hasn't finished, self-clearing via polling once it does (and now also self-clearing on a timeout if it never does) — reasoned safe to show ahead of any actual Gemini output, since a processing indicator carries none of the "is this suggestion any good" risk actual AI-generated content does. The worker (`app/ai_jobs/main.py`) now actually runs persistently rather than only being invoked manually for testing, and a one-time backfill (`scripts/backfill_extraction_jobs.py`) caught up every document that predated the pipeline. Running the backfill surfaced a real, pre-existing, unrelated gap: **most of the original seeded demo documents (51 of 55) have no actual file content on disk** — `scripts/seed.py` only ever created database rows, never wrote real bytes — so Download/Preview for those has likely always silently 404'd too; see §10. No Gemini integration yet — no suggestions, no summaries, no chat.
- ⬜ Deployment
...
- **Most of the original seeded demo documents have no real file on disk — `scripts/seed.py` only ever created database rows.** Discovered 2026-07-24 while backfilling extraction jobs for the AI feature track: 51 of 55 documents' current versions failed with `FileNotFoundError` on their stored `storage_path` (5 succeeded — exactly the documents uploaded through the real running app during manual testing this session, not seed data). The seed script fabricates realistic `documents`/`document_versions` rows (filenames, checksums, sizes) but has apparently never written the corresponding bytes to `uploads/documents/`. This almost certainly means **Download and Preview have been silently broken for most of the seed corpus since Phase 2** — this is simply the first feature that ever tried to read every document's file rather than a human spot-checking a handful during testing. Not yet fixed: would need `scripts/seed.py` to either generate or bundle real fixture content (matching each document's declared `mime_type` — plain text isn't enough for a `.pdf`/`.docx`/`.xlsx`/`.pptx` row to open correctly) for ~51 documents, a distinct and nontrivial task. No current feature is blocked by this (extraction fails cleanly for these and stays that way; the AI-track badge correctly never shows for documents this old regardless), but it's worth fixing before this app is used in an actual demo where someone might click Download on a random seeded document. **High priority relative to other items in this list**, given the blast radius.
