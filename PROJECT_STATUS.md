...
**Last updated:** 2026-07-24 (AI feature track — first user-facing surface: a quiet "Analyzing document…" badge on Document Details, self-clearing via polling; still pre-Gemini)
...
| Text Extraction *(AI track)* | ➖ still no dedicated router — `extractionStatus` rides the existing `GET /documents/{id}` instead | ✅ `TextExtractionService` | ✅ `DocumentExtractedTextRepository` + `AiJobRepository` | Fully wired end-to-end: upload/new-version/restore enqueue automatically, the worker (`app/ai_jobs/main.py`) processes them, hard-delete cleans up, and `DocumentDetail` now exposes `extractionStatus` for the frontend badge. Verified live against the real API + a real worker run, not just `pytest` (6 tests). |
